...
**Confirmed at Task 15 (August 2026):** packaged from a temp copy of `skill/` renamed to `inq-skill-builder/` (with the provisional-description comment stripped per `frontmatter.md`), producing `inq-skill-builder-v1.zip`. Unzipping it puts `inq-skill-builder/` at the zip root with `SKILL.md` directly inside — no wrapping subfolder, folder name matches frontmatter `name`, `quick_validate` passes on the unzipped copy. Layout matches this file's prediction exactly; no surprise. Installed to `~/.claude/skills/inq-skill-builder/` and confirmed the skill triggers in a fresh headless Claude Code session in a neutral directory (`claude -p "I want to turn my usability-testing debrief process into a claude skill"` invoked the `inq-skill-builder` Skill tool and followed the scan → interview flow from `elicitation.md`). Claude.ai upload itself is a manual step — untested here.
